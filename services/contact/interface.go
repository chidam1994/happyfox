...
type Repository interface {
	...
	RemEmail(contactId uuid.UUID, email *models.Email) error
	RemPhNum(contactId uuid.UUID, email *models.PhNum) error
	Delete(contactId uuid.UUID) error
	Find(filterMap map[string]string) ([]*models.Contact, error)
